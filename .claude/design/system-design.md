...
| D24 | Cost cap: Phase 1 soft warning at 60% of the budget + configurable hard cap in `.claude/design/cost-policy.md` (default $50/session) + granular per-invocation log with tokens in/out and estimated cost in `.claude/state/costs.jsonl` + tokens→USD table with last-updated date and source | Resolution of Council G8: a warning without enforcement gets ignored; a very high hard cap is a safety net against pathologies without friction in normal use. **Implementation status (2026-08-31): not built.** No `PostToolUse` or cost-accounting `PreToolUse` hook is wired, so `costs.jsonl` is never written and neither the 60% warning nor the hard cap exists. The decision stands as a specification; see the status block in `cost-policy.md`. |
...
│   │   ├── costs.jsonl                    # granular tokens/USD log (D24) — NOT IMPLEMENTED, no writer exists
