...
| 6 | Modification of hooks or any settings file that alters permission policy | path matches `\.claude/settings(\.local)?\.json` or `\.githooks/.*` | Changes the security model. **`settings.local.json` is included and this is not cosmetic**: it has *higher* precedence than the project `settings.json`, so a single write there can override the protected file — including `"disableAllHooks": true`, which switches every hook off. Protecting only `settings.json` left the higher-precedence file open. |
...
| 10 | Modification of the layer that governs the master's own behaviour (`CLAUDE.md`, `.claude/scripts/`, `.claude/hooks/`, `.claude/commands/`, `.claude/skills/`) | path matches `(CLAUDE\.md\|\.claude/(scripts\|hooks\|commands\|skills)/)` | The layer that enforces this list was not protecting itself: the master's own instructions, the matcher script, and the hooks could be edited at L1, which would let the system silently disable its own guardrail. `commands/` and `skills/` belong here for the same reason — they are instruction surfaces into the orchestrator. `.claude/commands/flipper-quick.md` is literally the file that tells the master when it may skip the Council; leaving it editable at L1 meant the rule for bypassing deliberation could be rewritten without deliberation. |
